...
def save_fixes_to_db(data_path: str = "data"):
  ...
  for fix_path in tqdm(data_fixes, total=len(data_fixes), desc="Storing fixes to MongoDB"):
...
def save_data_to_db(src_path: str = "data/GVD2022.zip", dont_save_fixes: bool = False):
  ...
  for name in tqdm(archive.namelist(), total=len(archive.namelist()), desc="Storing data to MongoDB"):
